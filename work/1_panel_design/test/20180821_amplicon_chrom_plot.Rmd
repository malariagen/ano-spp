...
```{r}
ampls <- read.csv(file = '/Users/am60/malaria/20180129_phylo_ampliseq/20180406_amplicon_data.csv')
ampls
```
...
```{r}
ampl_coord$labels <- paste(c(0:64, "65-73", rep('',8)))
kp <- plotKaryotype(genome=agam_gr)
kpAddMainTitle(kp, main="Distribution of amplicons in AgamP4 chromosomes")
kpAddBaseNumbers(kp)
kpPoints(karyoplot=kp, data=ampl_coord, y=0.04)
kpPlotMarkers(kp, data = ampl_coord, text.orientation = "horizontal", r1=0.8, line.color = "#AAAAAA", marker.parts=c(0.2, 0.6, 0.2))
```
